...
export const isLevelPassed = (userId, levelId, userAnswer) => {
  ...
  return Promise.all([User.findById(userId).lean(), Level.findById(levelId).lean()])
    .catch((error) => {
        ...
        return Level.find()
          ...
          .then((levelIds) => {
            ...
            if (!user.generalProgress.find((levelObjectId) => levelObjectId._id.toString() === levelId)) {
              const { generalProgress } = user

              generalProgress.push(level._id)

              const score = user.score + level.difficulty

              const currentLevel = (user.currentLevel = levelIds[index + 1])

              return User.updateOne({ _id: userId }, { $set: { generalProgress, currentLevel, score } }).catch((error) => {
                throw new SystemError(error.message)
              })
            }
          })
