...
export function Level({ level }) {
  ...
  return (
    <div className='bg-white bg-opacity-90 border border-purple-200 rounded-xl p-4 shadow-md transition-all'>
      {view === 'closed' && (
        <button onClick={handleLevelClick} className='w-full text-left text-purple-900 font-semibold hover:text-purple-700'>
          ✨ {level.name}
        </button>
      )}
      ...
      {view === 'inGame' && (
        <section className='mt-2 space-y-2'>
          ...
          <form onSubmit={handleResponseClick} className='space-y-2 mt-2'>
            ...
            <button className='bg-purple-500 hover:bg-purple-600 text-white py-1 px-4 rounded-full'>Enviar respuesta</button>
          </form>
        </section>
      )}
