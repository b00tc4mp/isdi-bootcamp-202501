...
const withErrorHandling = callback => {
    return (req, res, next) => {
        try {
            callback(req, res)
                .catch(error => next(error)); // Captura errores en promesas y los pasa al middleware de error
        } catch (error) {
            next(error); // Captura errores síncronos y los pasa al middleware de error
        }
    };
};
    ...
    .then(() => {
        ...
        api.post('/users/auth', jsonBodyParser, withErrorHandling((req, res) => {
            ...
            return logic.authenticateUser(username, password)
                .then(id => {
                    const token = jwt.sign({ sub: id }, JWT_SECRET); // Genera un JWT con el ID del usuario
                    res.json({ token });
                });
        }));
        ...
        // Endpoint para obtener posts del usuario autenticado
