...
import cors from 'cors' // Middleware para permitir solicitudes desde otro orígenes (Cross-Origin Resource Sharing)
import { errors } from 'com' // Importamos error personalizados desde un módulo 
...
// Definimos una clave secreta para firmar los token JWT
...
const handleWithErrorHandling = (next, callback) => {
    try{
        ...
        console.error(error) // Si hay un error en el try, lo capturamos aquí.
        next(error) // Pasamos el error al middleware de manejo de errores
    }
}

// Conectamos con la base de datos MongoDB
data.connect('mongodb://127.0.0.1:27017', 'test') // URL local de MongoDB y nombre de la base de datos
    .catch(console.error) // Captura errores en la conexion
    .then(() => { // Si la conexión es exitosa, arrancamos la API
        ...
        api.post('/users', jsonBodyParser, (req, res, next) => {
            handleWithErrorHandling(next, () => {
                ...
                return logic.registerUser(name, email, username, password) // LLamamos a la lógica de registro
                    .then(() => res.status(201).send()) // Si todo va bien, devovlemos código 201 (creado)
            })
        })
        ...
        api.get('/users/self/name', (req, res, next) => {
            handleWithErrorHandling(next, () => {
                ...
                const token = authorization.slice(7) // Quiramos 'Bearer ' del token
        ...
        api.patch('/posts/:postId/likes', (req, res, next) => {
            handleWithErrorHandling(next, () => {
                ...
                return logic.toggleLikePost(userId, postId) // Alteramos el like del post
