...
const interface = {
    askNumber(){
        data.playerNumber = prompt('What number is it? (0 to 100 both included)') //preguntamos numero
        if(data.playerNumber === null) data.booleans.attempsPassed = true
        logic.numbersTriedFunction()
        try{
            logic.askNumber(data.playerNumber)
            
        }catch(error){
            alert(error.message)
            this.askNumber(); // Volver a preguntar si no es válido
        }
    },

    checkAttempts(){
        try{
            const checkAttempsMaxim = logic.checkAttempts()
    ...
    checkNumber(){
        try{
            const messageForUsuari = logic.checkNumber()
    ...
    playAgain(){
        const wannaPlayAgain = prompt("Wanna play again? y/n") // Preguntar al usuario si quiere volver a jugar
        try{
            logic.playAgain(wannaPlayAgain)
             // Si no quieren  jugar mas capturaremos el valor que retornamos de la logica en esta variable.
            alert('thanks for playing') // Y lo mostraremos en un alert.
        }catch(error){
            alert(error.message)
            this.playAgain() // Volver a preguntar si no es válido
        }
    }
}
