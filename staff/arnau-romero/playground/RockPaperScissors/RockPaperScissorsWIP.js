...
function singleOrMultiplePlayerLogic (gameMode){
...
*/
    ...
    if(reGexGameMode.test(gameMode)){ //Si coincide entramos en este if
        ...
        if(gameMode === 's'){
            ...
            RockPaperScissorsPlayer1Presentation()
            ...
            RockPaperScissorsPlayer1Presentation()
            RockPaperScissorsPlayer2Presentation()
            checkPlay()
            roundsRemainingFunction()
        ...
        throw new Error("Please only characters 's' or 'm'!!") // -> { message: 'invalid word' }
    }
       
}
    
function RockPaperScissorsPlayer1Logic(player1choice){
//Reglas de la funcion : Solo poner 'r' 'p' o 's' [ru3a.2]
 if(reGexChoisePlayer.test(player1choice)) return // Si es true salimos de la funcion
    ...
    throw new Error('Please only "r", "p" or "s" ')    
 }
}

function RockPaperScissorsPlayer2Logic(player2choice){
//Reglas de la funcion : Solo poner 'r' 'p' o 's' [ru3a.2] 
...
function roundsRemainingFunctionLogic(){
    return currentRounds == maxRounds
   
}
//PRESENTACION

function roundsRemainingFunctionPresentation(){

}

function SingleOrMultiplePlayerPresentation (){ //- Funcion para elegir modo un jugador o dos jugadores.[u1]
...
try {
    singleOrMultiplePlayerLogic(gameMode)
} catch (error) {
    alert(error.message)

    console.error(error)

    SingleOrMultiplePlayerPresentation()
}
NumberRounds()
}

function NumberRounds(){ //- El usuario eligira cuantas rondas quiere jugar. [u2]
    // Preguntar el numero de rondas a jugar. [u2.1]
    maxRounds = prompt ('How many rounds do you wanna play? ')
}
function RockPaperScissorsPlayer1Presentation(){ //- El usuario eligira si saca piedra papel o tijera. [u3a]
  //Primero habra que preguntarle al usuario 1 que quiere sacar y almacenarla en una variable. [u3a.1]
    ...
    try{
        RockPaperScissorsPlayer1Logic()
    }catch (error){
        alert(error.message)
        RockPaperScissorsPlayer1Presentation()
    }
}
function RockPaperScissorsPlayer2Presentation(){ //- El usuario eligira si saca piedra papel o tijera. [u3a]
    //Primero habra que preguntarle al usuario 1 que quiere sacar y almacenarla en una variable. [u3a.1]
    player2choice = prompt('Player 1 turn: Rock(r) Paper(p) or Scissors(s) ? ') //Preguntar piedra papel o tijera [u3a.1]
    player2choice = player2choice.toLowerCase() // Pasarlo a minusculas por si lo pone en mayusculas
    
    try{
        RockPaperScissorsPlayer2Logic()
    }catch (error){
        alert(error.message)
        RockPaperScissorsPlayer2Presentation()
    }
}
function checkplayPresent(){
...
function roundsRemainingFunctionPresentation(){ // Cuantas rondas quedan. [m3]
