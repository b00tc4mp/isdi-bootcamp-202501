var logic = {
  ...
  helper: {
    checkWordMatchesAlphabet: function (word) {
      ...
      for (var i = 0; i < word.length; i++) {
        ...
        var characterInAlphabet =
          logic.helper.checkCharacterMatchesAlphabet(candidateCharacter);
    ...
    checkCharacterMatchesAlphabet: function (character) {
      for (var i = 0; i < logic.constants.ALPHABET.length; i++) {
        ...
        if (character === alphabetCharacter) return true;
      }
    ...
    validateCharacterMatchesAlphabet: function (character, explain) {
      if (!logic.helper.checkCharacterMatchesAlphabet(character))
        throw new SyntaxError(
          "invalid " + explain + " syntax (not in alphabet)"
        );
    },
  ...
  introduceWord: function (candidateWord) {
    ...
    for (var i = 0; i < data.word.length; i++) data.progress[i] = false;
  },
  ...
  attemptCharacter: function (attemptedCharacter) {
    ...
    logic.helper.validateTextIsCharacter(
      attemptedCharacter,
      "attemptedCharacter"
    );
    logic.helper.validateCharacterMatchesAlphabet(
      attemptedCharacter,
      "attemptedCharacter"
    );

    if (!logic.helper.hasRemainingAttemps()) throw new Error("no more attemps");
  ...
  attemptWord: function (attemptedWord) {
    ...
    if (attemptedWord === data.word)
      for (var i = 0; i < data.progress.length; i++) data.progress[i] = true;
    else data.remainingAttemps = 0;
  },
