...
export const getPosts = userId => {
    ...
    return User.findById(userId).lean()
        ...
        .then(posts => { //Este .then queda mas elegante ponerlo fuera del anterior
            ...
            return User.find({ _id: { $in: authors } }).lean() //Busca todos los usuarios cuyos id este dentro del array authors. Esto devuelve un cursor, por eso tuvimos que agregar el .toArray()
                .catch(error => { throw new SystemError(error.message) })
