...
export function Diary() {
  ...
  if (selectedEntry) {
    return <DiaryEntryDetail entry={selectedEntry} onClose={() => handleSelectEntry(null)} onUpdated={loadEntries} />
  }
    ...
    <div className='min-h-screen bg-pink-100 p-6'>
      <div className='max-w-md mx-auto space-y-6'>
        ...
        <div className='max-w-md mx-auto bg-white rounded-3xl p-6 shadow space-y-4'>
          <div className='space-y-3 max-h-[60vh] overflow-y-auto px-2 py-4 scrollbar-none'>
            {entries.map((entry) => (
              <div key={entry.id} onClick={() => handleSelectEntry(entry)} className={`cursor-pointer p-4 rounded-xl shadow transition hover:scale-[1.01] ${entry.own ? 'bg-pink-400/80 text-white font-semibold' : 'bg-pink-100 text-black'}`}>
