const logic = {
  ...
  helpers: {
    ...
    typeStringError(value) {
      if (typeof value !== "string") throw new TypeError("Type error of entry");
    },
    typeNumberError(value) {
      if (typeof value !== "number") throw new TypeError("Type error of entry");
    },
    emptyError(value) {
    ...
    gameOver(rondas, sum) {
      ...
      if (
        (rondas.length >= rounds && (sum < -1 || sum > 1)) ||
        (rondas.length >= rounds && (sum < 0 || sum > 0))
      )
 ...
 selectRounds(rounds) {
  ...
  this.helpers.typeNumberError(rounds);
   
   data.rounds = rounds;
 },

  takeDesicion(value) {
    this.helpers.typeStringError(value);
    this.helpers.handleError(value);

    var sum = 0;

    for (i = 0; i < data.rondas.length; i++) sum += data.rondas[i];
