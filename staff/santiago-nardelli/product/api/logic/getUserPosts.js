...
export const getUserPosts = (userId, userSearchId) => {
  validate.id(userId, "userId"); //==> valido que userId sea un id
  validate.id(userSearchId, "userSearchId"); //==> valido que userSearchId sea un id

  return Promise.all([
    User.findById(userId).lean(), //==> busco el usuario por id y lo convierto a un objeto plano
    User.findById(userSearchId).lean(),
    Post.find({author: userSearchId}).select('-__v').sort('-createdAt').populate("author", "name").lean(),
  ])
    .catch(error => {throw new SystemError(error.message)})
    .then(([user,userSearch, posts]) => {
      if (!user) throw new NotFoundError(`User with id ${userId} not found`);
      if (!userSearch) throw new NotFoundError(`User with id ${userSearchId} not found`); //==> valido que el usuario exista
