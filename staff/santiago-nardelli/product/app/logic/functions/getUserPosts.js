...
import { errors, validate } from "com"; // Importa el objeto `errors`, que contiene clases de error personalizadas.

const { SystemError } = errors; // Desestructura `SystemError` de `errors` para usarlo más tarde.

export const getUserPosts = (userSearchId) => {
  validate.id(userSearchId, 'userSearchId'); // Valida el `userSearchId` para asegurarse de que sea un ID válido.
  const { token } = data; // Obtiene el `userId` de los datos importados, que se usará para la autenticación en la solicitud.

  return fetch(`http://localhost:3000/user/${userSearchId}/posts`, {
