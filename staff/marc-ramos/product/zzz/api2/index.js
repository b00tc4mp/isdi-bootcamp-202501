...
api.delete('/delete', jsonBodyParser, (req, res) => { // creamos un nuevo post, el jsonBodyParser se utiliza como middleware para asegurarse de que el cuerpo de la solicitud esté en formato json
    try {
        const { userId, postId } = req.body // extraemos los datos del cuerpo de la solicitud

        logic.deletePost(userId, postId) // eliminamos una publicacion

        res.status(200).send() // si no ocurre ningún error, pasamos un 201 conforme se ha creado correctamente
    } catch (error) {
        console.error(error)
...
api.get('/posts', jsonBodyParser, (req, res) => { // creamos un nuevo post, el jsonBodyParser se utiliza como middleware para asegurarse de que el cuerpo de la solicitud esté en formato json
    try {
        const { userId } = req.body // extraemos los datos del cuerpo de la solicitud
...
api.patch('/toggle', jsonBodyParser, (req, res) => { // creamos un nuevo post, el jsonBodyParser se utiliza como middleware para asegurarse de que el cuerpo de la solicitud esté en formato json
    try {
        const { userId, postId } = req.body // extraemos los datos del cuerpo de la solicitud
...
api.patch('/updatet', jsonBodyParser, (req, res) => { // creamos un nuevo post, el jsonBodyParser se utiliza como middleware para asegurarse de que el cuerpo de la solicitud esté en formato json
    try {
        const { userId, postId, text } = req.body // extraemos los datos del cuerpo de la solicitud
