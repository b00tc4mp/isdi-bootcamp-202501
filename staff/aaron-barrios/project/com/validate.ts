...
export const validate = {
    ...
    routineWorkouts(value: any[], minLength: number, explain: string = "workouts") {
        if (!Array.isArray(value)) throw new ValidationError("Invalid workouts array")
        if (value.length < minLength) throw new ValidationError(`You must add at least ${minLength} ${explain}`)
    }
}