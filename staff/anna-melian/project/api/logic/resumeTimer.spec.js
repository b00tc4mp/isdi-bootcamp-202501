...
describe('resumeTimer', () => {
    ...
    it('succeeds on existing user and timer', () => {
        ...
        return Promise.all([User.create({
            ...
            .then(() => Promise.all([
                Timer.create({ author: user._id, time: 50, startDate: new Date, pauseTime: 3, pausesCount: 1, tag: 'Study', status: 'pause', createdAt: new Date(2025, 1, 11) })
            ]))
            .then(([_timer]) => {
                timer = _timer
            })
            .then(() => resumeTimer(user.id, timer.id))
    ...
    it('succeeds on existing user and timer in extraTime', () => {
        ...
        return Promise.all([User.create({
            ...
            .then(() => Promise.all([
                Timer.create({ author: user._id, time: 50, startDate: new Date, pauseTime: 3, pausesCount: 1, extraTimes: [10], tag: 'Study', status: 'pause', createdAt: new Date(2025, 1, 11) })
            ]))
            .then(([_timer]) => {
                timer = _timer
            })
    ...
    it('falis on non-existing user', () => {
        ...
        return User.create({
            ...
            .then(() => resumeTimer(new ObjectId().toString(), timer._id.toString()))
    ...
    it('fails on invalid user id', () => {
        ...
        return User.create({
            ...
            .then(_timer => timer = _timer)
            .then(() => {
                expect(() => resumeTimer(123, timer._id.toString())).to.throw(ValidationError, 'invalid userId type')
                expect(() => resumeTimer('123', timer._id.toString())).to.throw(ValidationError, 'invalid userId length')
                expect(() => resumeTimer(' '.repeat(24), timer._id.toString())).to.throw(ValidationError, 'invalid userId syntax')
            })
    ...
    it('fails on invalid timer id', () => {
        ...
        return User.create({
            ...
            .then(_timer => timer = _timer)
            .then(() => {
                expect(() => resumeTimer(user._id.toString(), 123)).to.throw(ValidationError, 'invalid timerId type')
                expect(() => resumeTimer(user._id.toString(), '123')).to.throw(ValidationError, 'invalid timerId length')
                expect(() => resumeTimer(user._id.toString(), ' '.repeat(24))).to.throw(ValidationError, 'invalid timerId syntax')
            })
    ...
    it('fails on user is not the timer owner', () => {
        ...
        return Promise.all([
            ...
            .then(() => Timer.findOne({ createdAt: new Date(2025, 1, 11) }).lean())
            .then(_timer => timer = _timer)
            .then(() => resumeTimer(user._id.toString(), timer._id.toString()))
            .catch(error => catchedError = error)
            .finally(() => {
    ...
    it('falis on timer is not paused', () => {
        let catchedError
        let timer
        let user
        return User.create({
            ...
            .finally(() => {
                expect(catchedError).to.be.instanceOf(TimerError)
                expect(catchedError.message).to.equal('timer is not paused')
            })
